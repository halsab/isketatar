...
Ниже текста — «Текст укылды» как явное действие, вопросы и результаты. Отметка прочтения сохраняется независимо от ответов. Позиция сохраняется автоматически по смысловому якорю; отдельная кнопка закладки обозначает намерение пользователя, а не любое прокрученное слово. Выделение и копирование не запускают словарь после протягивания мыши/пальца. У строки есть «Сүз сайларга»: клавиатурное раскрытие списка точных вхождений в потоке, без чтений и значений, с последующим открытием той же word-панели.
