...
| Валидация | JSON Schema + Ajv standalone для импорта и пакетов; компиляция при сборке, готовые функции в runtime | Схемы являются границей недоверенных данных, не генератором архитектуры |
...
Ajv compile не вызывается в браузере: [standalone-валидаторы](https://ajv.js.org/standalone.html) генерируются при сборке и не требуют new Function/unsafe-eval. Это согласуется с CSP из [09](09-security-reliability.md).
...
3. Читаются технический реестр PWA и manifest встроенного release, проверяется принятый current и незавершённый update_gate. Hard reload новой оболочки не разрешает ей мигрировать общую БД до согласования выпуска. Существующая БД сначала открывается без запроса повышения версии для ограниченного чтения control; неподдерживаемая схема не изменяется. Создание новой БД разрешается лишь после проверки выбранного release.
4. Совместимый принятый reader открывает прогресс; необходимая versionchange-миграция выполняется только в согласованном запуске/commit обновления по 08. blocked ведёт к экрану закрытия другой вкладки, сбой не удаляет базу. При недоступном постоянном хранилище курс продолжает работу в памяти с ограничениями 09, видимым статусом и экспортом. Затем загружается каталог закреплённого совместимого пакета; нет данных — экран повтора, есть сохранённый пакет — работа с ним.
5. После проверки версии восстанавливается сессия или запрошенный deep link. Onboarding показывается по необходимости, не перехватывает навсегда ссылку на справочник. Несогласованная новая оболочка предлагает согласование/возврат к принятому reader, не выполняет пользовательские команды.
