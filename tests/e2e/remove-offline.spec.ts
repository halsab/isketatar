...
test('removal preserves progress and shells, removes staged release, and fences an old download request', async ({ page, context }) => {
  ...
  releases.setUnavailable(true); await viewer.goto(releases.url + 'recovery.html'); await expect(viewer.getByRole('heading', { name: 'Курс ачылмады', exact: true })).toBeVisible();
  ...
  releases.setUnavailable(false); await viewer.close(); await save(page);
});
