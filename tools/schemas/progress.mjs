...
const nullable = schema => {
  if (schema.$ref && ['timestamp', 'identifier', 'uuid', 'hash'].some(name => schema.$ref === `#/$defs/${name}`)) return ref(`nullable_${schema.$ref.split('/').at(-1)}`);
  if (schema.type) return { ...schema, type: [schema.type, 'null'] };
  if (schema.enum) return { enum: [...schema.enum, null] };
...
const defs = {
  policy,
  timestamp: integer(0, 8_640_000_000_000_000), identifier: text(),
  uuid: { ...text(36), pattern: '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' },
  hash: { ...text(64), pattern: '^[0-9a-f]{64}$' },
  answer: { type: 'object', discriminator: { propertyName: 'kind' }, oneOf: [object({ kind: enumeration('unknown') }), object({ kind: enumeration('option'), option_id: id }), object({ kind: enumeration('set'), option_ids: array(id, 256, true) }), object({ kind: enumeration('text', 'segments'), text: answerText })] },
  ...
  session: object({ session_id: uuid, kind, status: enumeration('active', 'paused', 'submitted', 'abandoned', 'incompatible'), revision: integer(), data_generation: uuid,
    release_id: text(256), content_version: text(), content_schema: integer(1), policy_versions: ref('policy'), lesson_id: nullable(id), reading_ids: array(id, 1000, true), diagnostic_imla_deferred: bool,
  ...
  attempt: object({ presentation_id: uuid, session_id: uuid, question_id: id, grading_revision: hash, ordinal: integer(1, 200000), release_id: text(256), content_version: text(), policy_versions: ref('policy'),
...
for (const name of ['timestamp', 'identifier', 'uuid', 'hash']) defs[`nullable_${name}`] = { ...defs[name], type: [defs[name].type, 'null'] };
...
defs.legacy = { type: 'object', discriminator: { propertyName: 'origin_kind' }, oneOf: Object.entries(history).map(([origin, name]) => object({ legacy_id: text(512), origin_kind: enumeration(origin), original_id: text(256), source_content_version: text(), reason: enumeration('unknown_content_id', 'unknown_grading_revision', 'unknown_policy_version', 'incompatible_draft'), record: ref(name), imported_at: time })) };
...
export const progressValidators = { validateControl: 'control', validateExport: 'export', validateData: 'data', validateMeta: 'meta', validateSession: 'session', validatePresentation: 'presentation', validateAttempt: 'attempt', validateExposure: 'exposure', validateReviewCard: 'reviewCard', validateBookmark: 'bookmark', validatePosition: 'position', validateLegacy: 'legacy' };
