...
const settings = {
  ...
  throttling: { cpuSlowdownMultiplier: 4, requestLatencyMs: 150, downloadThroughputKbps: 1600, uploadThroughputKbps: 750 },
  ...
  for (const [name, route, ready] of routes) {
    ...
    for (let run = 1; run <= count; run++) {
      ...
      try {
        ...
        const metrics = { lcp_ms: lhr.audits['largest-contentful-paint'].numericValue, tbt_ms: lhr.audits['total-blocking-time'].numericValue, cls: lhr.audits['cumulative-layout-shift'].numericValue };
    ...
    report.routes.push({ name, route, runs, median: { lcp_ms: median('lcp_ms'), tbt_ms: median('tbt_ms'), cls: median('cls') } });
