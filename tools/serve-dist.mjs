...
export async function serveDist(root = 'dist') {
  ...
  const server = createSecureServer({ key, cert }, (request, response) => {
    ...
    response.writeHead(200, { 'Content-Type': file.mime, 'Content-Length': body.length, 'Cache-Control': 'no-store', Vary: 'Accept-Encoding', ...(gzip ? { 'Content-Encoding': 'gzip' } : {}) });
    response.end(request.method === 'HEAD' ? undefined : body);
  });
  server.on('session', session => { sessions.add(session); session.on('close', () => sessions.delete(session)); });
  await new Promise((resolve, reject) => { server.once('error', reject); server.listen(0, '127.0.0.1', resolve); });
  return { url: `https://127.0.0.1:${server.address().port}/isketatar/`, spki, close: () => new Promise((resolve, reject) => { server.close(error => error ? reject(error) : resolve()); for (const session of sessions) session.destroy(); }) };
}
